...
def load_meta(opt, meta_path):
    ...
    opt['pos_vocab_size'] = len(meta['vocab_tag.tok2ind'])
    opt['ner_vocab_size'] = len(meta['vocab_ner.tok2ind'])
    opt['vocab_size'] = len(meta['vocab.tok2ind'])
    vocab = Vocabulary(meta['vocab.neat'])
    vocab.ind2tok = meta['vocab.ind2tok']
    vocab.tok2ind = meta['vocab.tok2ind']
    return embedding, opt, vocab
