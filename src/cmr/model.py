...
class DocReaderModel(object):
    ...
    def predict(self, batch, pick_tokens):
        ...
        for step in range(max_len):
            ...
            next_token = pick_tokens(torch.exp(log_prob))[:,0]
            next_token_np = next_token.data.cpu().numpy()
            ix_EOS = 3
            for i in range(batch_size):
                ix = int(next_token_np[i])
                if not done[i]:
                    sum_logP[i] += log_prob[i, ix].item()
                    preds[i].append(ix)
                if ix == ix_EOS:
                    done[i] = True
        ...
        avg_logP = [sum_logP[i]/(len(preds[i]) + 1e-9) for i in range(batch_size)]
        return avg_logP, preds
